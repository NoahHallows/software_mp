...
matching_image_list = [
    ...
    [
        sg.Listbox(
            values=["rer"], enable_events=True, select_mode="LISTBOX_SELECT_MODE_MULTIPLE", key="matching_images", size=(40, 20)
        )
    ],
        ...
        if image_encodings:
            ...
            if results[0]:
                ...
                for target_face_location in target_face_locations:
                    ...
                    processed_images = progress_queue.get()
...
def end_event_loop(results):
    window = sg.Window("Face removal tool", end_layout)
    while True:
        event, values = window.Read()
        window["matching_images"].update(results)
...
while True:
    ...
    if event == "SUBMIT":
        ...
        #monitor_thread = multiprocessing.(target=monitor_progress, args=(window, total_images, progress_queue))
